...
/** One row in `llm-profiles.json` — API key is per provider, not global `.env`. */
...
export function readProfilesFile(): LlmProfilesFile {
  try {
    const txt = fs.readFileSync(profilesFilePath(), "utf8");
...
export function writeProfilesFile(data: LlmProfilesFile): void {
  fs.mkdirSync(path.dirname(profilesFilePath()), { recursive: true });
...
/**
 * If `llm-profiles.json` exists, overlay `process.env` for the active provider
 * so restarts keep per-provider config without re-saving `.env` by hand.
 */
export function hydrateEnvFromProfiles(): void {
  let data: LlmProfilesFile;
  try {
    const txt = fs.readFileSync(profilesFilePath(), "utf8");
    data = JSON.parse(txt) as LlmProfilesFile;
