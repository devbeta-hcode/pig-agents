...
const DEFAULT_READY_TIMEOUT_MS = 45_000; // 45s (slow machines)
...
export async function runSmartCommand(
  ...
  const killAsGroup = process.platform !== "win32";
...
function tryKill(child: ChildProcess, killAsGroup: boolean) {
  try {
    if (killAsGroup && child.pid !== undefined) {
...
export function killBackgroundProcess(pid: number): boolean {
  ...
  if (child) {
    tryKill(child, process.platform !== "win32");
